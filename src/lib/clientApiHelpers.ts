...
import { GetOrdersQueryFilter } from "./queryKeys";
...
export const postSingleImage = async (
  imageFile: File
): Promise<PostImageSuccessResponsePayload> => {
  return await ky
    .post("/api/images", {
      body: imageFile,
    })
...
export const getTowns = async (
  wilayaCode: number
): Promise<GetTownsSuccessResponsePayload> => {
 ...
 */
export const getSingleProduct = async ([_, productCode]: (
  | string
  | null
)[]): Promise<Product> => {
  try {
...
export const getProducts =
  async (): Promise<GetProductsSuccessResponsePayload> => {
...
): Promise<PostProductSuccessResponsePayload> => {
  return await ky
    .post("/api/products", {
      json: data,
    })
...
): Promise<PostOrderSuccessResponsePayload> => {
  const orderId: PostOrderSuccessResponsePayload = await ky
    .post(`/api/orders`, {
      json: order,
    })
    .json();
...
): Promise<GetOrdersSuccessResponsePayload> => {
...
export const patchOrder = async (data: {
  code: string;
  status: Status;
}): Promise<{ code: string; status: Status }> => {
...
export const getCategories =
  async (): Promise<GetCategoriesSuccessResponsePayload> => {
    return await ky.get("/api/categories").json();
  };

export const postCategory = async (
  data: PostCategoryRequestPayload
): Promise<PostCategorySuccessReponsePayload> => {
  return await ky
    .post("/api/categories", {
      json: data,
    })
...
): Promise<PostSubCategorySuccessResponsePayload> => {
  return await ky
...
export const deleteCategory = async ({
  ...
  id: number;
  type: CategoryType;
}): Promise<{ id: number; type: CategoryType }> => {
  await ky
    .delete(
...
export const patchCategory = async ({
  ...
  id: number;
  name: string;
}): Promise<PatchCategorySuccessResponsePayload> => {
  const patchedData: PatchCategorySuccessResponsePayload = await ky
...
export const PatchSubcategory = async ({
  ...
  id: number;
  name: string;
}): Promise<PatchSubcategorySuccessResponsePayload> => {
  const patchedData: PatchSubcategorySuccessResponsePayload = await ky
